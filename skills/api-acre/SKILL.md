...
3. Read its free sample at `/samples/<service-slug>` and schema from `/catalog` only when needed.
...
- Remote MCP: `https://apiacre.com/mcp` (Streamable HTTP). Listing tools is free; a tool call may
  return an x402 requirement and still needs exact approval.
